class BSTNode:
    ...
    def display_aux(self):
        ...
        s = '%s' % self.val
        u = len(s)
        first_line = (x + 1) * ' ' + (n - x - 1) * ' ' + s + y * ' ' + (m - y -1) * ' '
        second_line = x * ' ' + '/' + (n - x + - 2 + u + y) * ' ' + '\\' + (m - y - 1) * ' '
        if p < q:
            left += [n * ' '] * (q - p)
        elif q < p:
            right += [m * ' '] * (p - q)
...
print("Saisir les élements de l'arbre ")
bst = BSTNode()
try:
    nums = []
...
for num in nums:
    bst.insert(num)
print("Saisir le(s) elements à ajouter")
try:
    numsk = []

    while True:
        numsk.append(int(input()))
except:
    for num in numsk:
        bst.insert(num)
    bst.display()
print("Saisir le(s) elements à effacer")
try:
    nums = []
